...
// per user request: increased (1.4->2.0, then 2.0->4.0, then 4.0->8.0) so friendly units don't
// start the wave already clustered close enough for their labels to overlap -- a complementary
// measure alongside drawPendingFriendlyLabels()'s label-clustering in render2d.js, since
// deployment spacing alone doesn't help once units regroup mid-battle.
export const INITIAL_DEPLOY_SPACING_MULT = 8.0;
...
export const COMBAT_CALLOUTS = {
  ...
  mortarFire: ['撃てー！','てーーッ！','半装填準備よし！','各個に撃て！','指名！','装填待て！'],
  ...
  irritation: ['動きが遅い、置いていくぞ！','なんでそんな場所に伏せてるんだ！','合図を見逃すな、集中しろ！','お前の射撃、味方に当たりそうだったぞ！','勝手に持ち場を離れるな！','そこは危険地帯だと言っただろ！','装備を忘れるとかあり得ない！','無線のチャンネル、間違えてるぞ！','お前、ちゃんと周り見てるのか！','足を引っ張るなら下がってろ！','新人だからって甘えるな！','お前の判断、いつも遅すぎるんだよ！','もっと声を出せ、聞こえないぞ！','なんでそっちに勝手に進んだ！','連携取れてないぞ、しっかりしろ！','お前が動くたびに位置がばれるんだよ！','無駄弾使うな、節約しろ！','そんな装備で来るなんて信じられない！','お前、寝てないのか、しっかりしろ！','言われたことだけやってりゃいいんだよ！','点呼に遅れるとかあり得ないだろ！','装備の員数、ちゃんと数えたのか！','また始末書か、いい加減にしろ！','持ち場を離れるなら一声かけろよ！'],
  outburst: ['もう嫌だ、こんな戦争！','なんでこんな所で死ななきゃいけないんだ！','家族のところに帰りたいだけなんだ！','お前にこの気持ちが分かるか！','仲間を見捨てるなんてできない！','誰かのために死ぬなんて意味あるのか！','もう何を信じればいいんだ！','これが正義だって言うのか！','お前は何も分かってない！','戦友を失ってなお戦えって言うのか！','命令だから仕方ないなんて言うな！','俺たちは駒じゃない！','もう誰も死なせたくないんだ！','なんでこんな作戦を許可したんだ！','怒りをぶつける相手を間違えるな！','お前まで俺を疑うのか！','信頼できるのはお前だけなんだ！','もう限界だと言ってるだろう！','終わったらすべて話し合おう、今は戦え！','絶対に、みんなで生きて帰るぞ！','始末書ばっかり書かされて嫌になる！','こんな些細なことで評価落とされてたまるか！'],
  // per user request(陸上自衛隊の「服務事故」の多さをエッセンスとして): resolveServiceIncidents()
  // が軽微な服務事故を発生させた際の吹き出し用の短い台詞。
  serviceIncident: ['またやらかしたのか…始末書だぞ！','次は気をつけろよ、頼むから！','班長に見つかったらまずいって！','こういうのが積み重なるんだぞ！','勘弁してくれ、報告書がまた増える…','気の緩みが一番危ないんだぞ！','今度こそ気をつけますって言ったよな！','こんなことで評価下げたくないだろ！'],
};

// per user request(出現頻度を今の半分、表示時間を1.5倍に): 元は4000ms/0.5。
export const CALLOUT_DURATION_MS = 6000;

export const CALLOUT_OCCURRENCE_CHANCE = 0.25;

// per user request(陸上自衛隊の「服務事故」の多さをエッセンスとして): 深刻な事故ではなく、
// 忘れ物・遅刻・些細な諍いといった「あるある」レベルの軽微な服務事故を、時折発生する
// フレーバーイベントとして追加する。実害は始末書対応の小額の出費のみで、戦闘力には
// 一切影響しない。resolveServiceIncidents()参照。
export const SERVICE_INCIDENT_CHANCE_PER_TURN = 0.025;

export const SERVICE_INCIDENT_COST_RANGE = [20, 60];

export const SERVICE_INCIDENT_REPORTS = [
  '装備品の員数点検で不備が発覚、始末書。',
  '巡回中にうたた寝しているところを発見され、厳重注意。',
  '個人携帯品を紛失、始末書及び弁償。',
  '営内での些細な諍いにより、両者始末書。',
  '点呼に遅刻、始末書。',
  '無線交信要領の不備を注意されるも口答えし、始末書。',
  '装備の手入れ不足を検査で指摘され、再教育処分。',
  '許可なく持ち場を離れて間食、始末書。',
  '報告書の記入漏れが発覚、上申のやり直し。',
  '車両(装備)点検票への記入漏れ、始末書。',
];
