...
export function initGame(){
  ...
  state = {
    ...
    medals: 0,
    // per user request(陸上自衛隊の「服務事故」の多さをエッセンスとして): 軽微な服務事故の
    // 件数。ThisWaveはstartStage()でWAVEごとにリセットされ、WAVE CLEAR画面に表示する。
    // Totalはキャンペーン通算で保持し(savegame.jsで永続化)、作戦完了画面に表示する。
    serviceIncidentsThisWave: 0,
    serviceIncidentsTotal: 0,
    ammo: {he:0, heat:0},
    ...
    selectedId: null,
    commandBox: null,
    // per user request(ドラクエ風の階層コマンドメニュー): 開いているコマンドボックスの中で
    // 現在表示中の大分類(命令/攻撃目標/修理/その他等、ユニット種別ごとに異なる)。nullなら
    // 大分類の選択肢そのものを表示する(トップレベル)。renderCommandBox()がユニット選択の
    // 切り替わりを検知して自動でnullに戻す(see lastCommandBoxKey in ui.js)。
    commandBoxMenu: null,
...
export function startStage(){
  ...
  state.illumFlares = [];
  state.mines = [];
  state.serviceIncidentsThisWave = 0;
...
// per user request(自軍戦車が強すぎるとの声を受けて追加): 敵対戦車部隊(resolveEnemyAntiTank)
// が狙う「地上装甲」だけを対象にした絞り込み版 -- nearestFriendlyVehicleLikeと違いヘリは
// 対戦車砲の射線が通る低空でも直接照準の主目的ではないため除外する。
function nearestFriendlyArmor(x, y, range){
  let best=null, bestDist=Infinity;
  state.tanks.forEach((tk,idx)=>{
    if(tk.hp<=0) return;
    const d = Math.hypot(tk.x-x, tk.y-y);
...
export function hasLineOfSight(fromX,fromY,toX,toY){
  ...
  // per user request(バグ調査): 森林による視線遮断は「遠距離からは森の奥が見通せない」を
  // 意図したものだが、距離を問わず無条件に適用されていたため、敵が森林タイル上にいると
  // 密着(包囲)していても永久に交戦不能・無傷のままになっていた。FOREST_LOS_MIN_RANGE_UNITS
  // 以内の至近距離では森林による遮断を無効化する。
  const forestBlocksLos = dist > FOREST_LOS_MIN_RANGE_UNITS;
  if(forestBlocksLos && terrainTypeAt(toX,toY)===TERRAIN_TYPE_FOREST) return false;
  ...
  for(let i=1;i<steps;i++){
    ...
    if(forestBlocksLos && terrainTypeAt(x,y)===TERRAIN_TYPE_FOREST) return false;
...
// per user request(コマンドボックスが地図を覆って移動先を指定しづらいとの声を受けて追加):
// 「移動」ボタンを押した瞬間、詳細なコマンドボックスは閉じ、最小限の「地点を指定せよ」
// プロンプト(renderMoveOrderPrompt、ui.js)に差し替える。DIRECT_MOVE_KINDSの全種別で共通
// (setUnitMoveDest自身が種別ごとの生存/休息チェックを行うので、ここでは無条件に許可する)。
export function armDirectMoveOrder(kind, idx){
  state.orderMode = {kind:'direct-move', unitKind:kind, idx};
  state.commandBox = null;
  render();
}

export function cancelDirectMoveOrder(){
  state.orderMode = null;
  render();
}

// per user request(ドラクエ風の階層コマンドメニュー): コマンドボックスの中身を「命令」
// 「攻撃目標」「修理」等の大分類ボタンだけのトップレベルと、選んだ大分類の中身(既存の
// ボタン/一覧をそのまま流用)の2階層に分ける。押した大分類をstate.commandBoxMenuに
// 記録するだけで、実際の各操作(setSquadOrder/assignSquadHunt/repairTank等)は一切
// 変更しない。
export function openCommandCategory(key){
  state.commandBoxMenu = key;
  render();
}

export function closeCommandCategory(){
...
export function enemyCounterAttack(dt){
  ...
  const hqThreat = findHqDefenseThreat();
  {
    remaining.forEach(t=>{
      ...
      // resolveEnemyAntiAir, not a generic ground-engagement roll here. 'at_gun' likewise --
      // a dedicated anti-tank system handled entirely by resolveEnemyAntiTank.
      if(t.type==='hq' || t.type==='aa' || t.type==='at_gun') return;
...
export function resolveTankOrders(dt){
  let anyEvent = false;
  {
    state.tanks.forEach((tank, idx)=>{
      ...
      // per user request(自軍戦車が強すぎるとの声を受けて追加): 敵対戦車部隊(at_gun)も
      // huntなしで通常交戦対象に含める -- resolveEnemyAntiTank側から一方的に狙われるだけ
      // になってしまうと発見しても反撃できず理不尽なため。
      let engageTargets = state.targets.filter(t=>!t.destroyed && (t.type==='infantry' || t.type==='vehicle' || t.type==='artillery' || t.type==='at_gun'));
...
export function resolveAntitankOrders(dt){
  let anyEvent = false;
  {
    state.antitanks.forEach((at, idx)=>{
      ...
      // 主兵装: 対戦車ロケットランチャー(vehicle/at_gunタイプ専任)。
      // per user request(自軍戦車が強すぎるとの声を受けて追加): 敵対戦車部隊(at_gun)も
      // 装甲目標として対戦車の主兵装の対象に加える。
      let engageTargets = state.targets.filter(t=>!t.destroyed && (t.type==='vehicle' || t.type==='at_gun'));
      if(at.order==='hunt' && at.huntTargetId){
        const huntTarget = state.targets.find(t=>t.id===at.huntTargetId && !t.destroyed && (t.type==='vehicle' || t.type==='at_gun'));
        if(huntTarget && !engageTargets.includes(huntTarget)) engageTargets = engageTargets.concat([huntTarget]);
      }
      let dmgMult=1, incomingMult=1;
...
// per user request(自軍戦車が強すぎるとの声を受けて追加): 敵対戦車部隊 -- resolveEnemyAntiAir
// と同じ「据え置き・専任・ターン単位のクールダウン」設計で、自軍の戦車/対戦車車両
// (nearestFriendlyArmor、ヘリは対象外)だけを狙う。ENEMY_AT_GUN_RANGEは戦車の
// TANK_ENGAGE_RANGE/対戦車のANTITANK_ENGAGE_RANGEより長く、一撃の重さも上回るが、
// ENEMY_AT_GUN_COOLDOWN_TICKSにより連射はできない。
export function resolveEnemyAntiTank(dt){
  const atGuns = state.targets.filter(t=>!t.destroyed && t.type==='at_gun');
  if(atGuns.length===0) return false;
  let anyEvent = false;
  atGuns.forEach(t=>{
    if(t._atGunCooldown === undefined) t._atGunCooldown = 0;
    if(t._atGunCooldown > 0){
      t._atGunCooldown -= dt;
      return;
    }
    const armor = nearestFriendlyArmor(t.trueX, t.trueY, ENEMY_AT_GUN_RANGE);
    if(!armor) return;
    if(!hasLineOfSight(t.trueX, t.trueY, armor.x, armor.y)) return;
    // The attack itself is a discrete once-per-turn event, so it only resolves the instant a
    // whole turn is crossed -- same pattern as every other enemy attack roll in this file.
    if(!turnJustCrossed()) return;
    anyEvent = true;
    const e = estPos(t);
    if(revealTarget(t)){
      log('op','斥候', `${t.id} が対戦車砲撃用意、<b>${t.def.label}</b>と識別。`);
    }
    if(rollExposureHit(getUnitExposure(armor))){
      const dmg = Math.round(rnd(ENEMY_AT_GUN_DMG[0], ENEMY_AT_GUN_DMG[1]));
      damageFriendlyAsset(armor, dmg, `${t.id}(対戦車部隊)の砲撃`);
      fireTracer(e.x, e.y, armor.x, armor.y, 280, 'cannon');
    } else {
      log('sys','回避', `${t.id}(対戦車部隊)の攻撃を受けたが、掩蔽率により被弾を免れた。`);
    }
    t._atGunCooldown = ENEMY_AT_GUN_COOLDOWN_TICKS;
  });
  return anyEvent;
}
...
export function resolveSquadAntiVehicle(dt){
  let anyEvent = false;
  {
    state.squads.forEach((sq, sqIdx)=>{
      ...
      state.targets.forEach(t=>{
        // per user request(自軍戦車が強すぎるとの声を受けて追加): 敵対戦車部隊(at_gun)も
        // 至近距離なら小隊の即興対戦車射撃(手榴弾・対戦車ロケット等)の対象に含める。
        if(t.destroyed || (t.type!=='vehicle' && t.type!=='at_gun')) return;
...
// per user request(陸上自衛隊の「服務事故」の多さをエッセンスとして追加): 深刻な事故では
// なく、忘れ物・遅刻・些細な諍いといった「あるある」レベルの軽微な服務事故を、時折発生する
// フレーバーイベントとして扱う。ランダムに選んだ生存中の自軍部隊1つに始末書対応の小額の
// 出費が発生するだけで、戦闘力(HP/exposure/standingOrder等)には一切影響しない。
export function resolveServiceIncidents(){
  if(!turnJustCrossed()) return false;
  if(Math.random() >= SERVICE_INCIDENT_CHANCE_PER_TURN) return false;
  const pool = FRIENDLY_KIND_LIST.filter(e=>e.list().some(u=>e.alive(u)));
  if(!pool.length) return false;
  const entry = choice(pool);
  const candidates = entry.list().map((u,i)=>i).filter(i=>entry.alive(entry.list()[i]));
  if(!candidates.length) return false;
  const idx = choice(candidates);
  const label = entry.label(idx);
  const report = choice(SERVICE_INCIDENT_REPORTS);
  const cost = Math.round(rnd(SERVICE_INCIDENT_COST_RANGE[0], SERVICE_INCIDENT_COST_RANGE[1]));
  state.money = Math.max(0, state.money - cost);
  state.serviceIncidentsThisWave = (state.serviceIncidentsThisWave||0) + 1;
  state.serviceIncidentsTotal = (state.serviceIncidentsTotal||0) + 1;
  log('sys','服務事故', `${label}、${report}(始末書対応費 ¥${cost})`);
  announceTicker(`${label} 服務事故発生`, 'warning');
  unitSpeak(entry.kind, idx, 'serviceIncident');
  return true;
}
...
export function resolveEnemyTurn(dt){
  ...
  const atGunEvent = resolveEnemyAntiTank(dt);
  ...
  // actively engaging.
  if(infEvent || antitankEvent || tankEvent || samEvent || antiDroned || heliEvent || aaEvent || atGunEvent || bandEvent) playCombatAmbience(); else stopCombatAmbience();
  state.targets.forEach(t=>{
